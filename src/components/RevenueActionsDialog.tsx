...
import { ListChecks, Loader2, RefreshCw, Copy, Check, Send } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useToast } from "@/hooks/use-toast";
import ReactMarkdown from "react-markdown";

interface RevenueActionsDialogProps {
  listingId: string;
  propertyName: string;
}
...
export function RevenueActionsDialog({ listingId, propertyName }: RevenueActionsDialogProps) {
  ...
  const generateRevenueActions = async () => {
    setLoading(true);
    setMessages([]);
    setDataContext(null);
    
    try {
      const { data, error } = await supabase.functions.invoke('generate-revenue-actions', {
        body: { listingId },
      });
      ...
      console.error('Error generating revenue actions:', error);
      toast({
        title: "Failed to generate revenue actions",
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Revenue actions have been copied",
      });
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            AI-generated action items for {propertyName}
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-4 flex-1">
            ...
            <p className="text-muted-foreground">Analyzing calendar, pricing, and market data...</p>
          </div>
        ) : messages.length > 0 ? (
          <div className="flex flex-col flex-1 min-h-0 gap-4 overflow-hidden">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" onClick={generateRevenueActions}>
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-4 flex-1">
            <p className="text-muted-foreground">Click the button below to generate action items</p>
            <Button onClick={generateRevenueActions}>
              <ListChecks className="mr-2 h-4 w-4" />
