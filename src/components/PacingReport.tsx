...
export function PacingReport({ reservations }: PacingReportProps) {
  ...
  const calculateCumulativeData = (): CumulativeDataPoint[] => {
    ...
    for (let month = 0; month <= currentMonth; month++) {
      ...
      // Calculate cumulative bookings up to this month for current year
      const currentYearCumulative = reservations.filter((r) => {
        if (!r.check_in) return false;
        const checkIn = parseISO(r.check_in);
        return checkIn.getFullYear() === currentYear && checkIn.getMonth() <= month;
      }).length;

      // Calculate cumulative bookings up to this month for last year
      const lastYearCumulative = reservations.filter((r) => {
        if (!r.check_in) return false;
        const checkIn = parseISO(r.check_in);
      ...
      <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-medium">{payload[0].value} bookings</span>
          </div>
          {payload[1] && (
            <div className="flex items-center gap-2 text-sm">
              <div className="w-3 h-3 rounded-full bg-secondary" />
              <span className="text-muted-foreground">{lastYear}:</span>
              <span className="font-medium">{payload[1].value} bookings</span>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Cumulative Booking Pace</CardTitle>
          <CardDescription>
            Running total of confirmed bookings - {currentYear} vs {lastYear}
          </CardDescription>
        </CardHeader>
        <CardContent>
