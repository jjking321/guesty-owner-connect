...
const CATEGORY_OPTIONS = [{
  ...
  value: 'unbookable_gap',
  label: 'Unbookable Gaps'
}, {
  value: 'low_rating',
  label: 'Low Rating'
}, {
  ...
  value: 'recent_low_review',
  label: 'Recent Low Reviews'
}, {
  value: 'missing_goals',
  label: 'Missing Goals'
}, {
...
export default function Actionables() {
  ...
  const {
    ...
    queryFn: async () => {
      const query = supabase.from('property_actionables').select(`
          *,
          listings (
            id,
            nickname,
  ...
  const refreshMutation = useMutation({
    mutationFn: async () => {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-actionables');
      if (error) throw error;
      return data;
    },
    onSuccess: data => {
      queryClient.invalidateQueries({
        queryKey: ['property-actionables']
      });
  ...
  const dismissMutation = useMutation({
    mutationFn: async (id: string) => {
      const {
        error
      } = await supabase.from('property_actionables').update({
        dismissed: true,
  ...
  return <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold">Actionables -UNDER CONSTRUCTION </h1>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center">
          ...
          <div className="flex gap-2">
            <Select value={categoryFilter} onValueChange={setCategoryFilter}>
              ...
              <SelectContent>
                {CATEGORY_OPTIONS.map(opt => <SelectItem key={opt.value} value={opt.value}>
                    {opt.label}
                  </SelectItem>)}
              </SelectContent>
            </Select>
            <Button variant={showDismissed ? "secondary" : "outline"} onClick={() => setShowDismissed(!showDismissed)}>
        ...
        {isLoading ? <div className="space-y-4">
          ...
          </div> : filteredActionables.length === 0 ? <div className="text-center py-12 bg-muted/50 rounded-lg">
            <h3 className="text-lg font-medium">No actionable items found</h3>
            <p className="text-muted-foreground mt-1">
              {searchQuery || categoryFilter !== 'all' ? 'Try adjusting your filters' : 'All properties are in good shape!'}
            </p>
          </div> : <div className="space-y-4">
            {filteredActionables.map((item, index) => <PropertyActionableCard key={item.id} rank={index + 1} propertyId={item.listing_id} propertyName={item.listings?.nickname || 'Unknown Property'} thumbnail={item.listings?.thumbnail} aggregateScore={item.aggregate_score} issues={item.issues} aiSummary={item.ai_summary} dismissed={item.dismissed} criticalCount={item.critical_count} highCount={item.high_count} mediumCount={item.medium_count} lowCount={item.low_count} onDismiss={() => dismissMutation.mutate(item.id)} isDismissing={dismissMutation.isPending} />)}
          </div>}
      </div>
    </DashboardLayout>;
}